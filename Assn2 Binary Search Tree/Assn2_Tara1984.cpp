...
class BSTNode{
	public:
		BSTNode( )			// Constructor. Any setup operation you wish for the class.
        {	}							// end constructor
		BSTNode(string d)	// Constructor. Any setup operation you wish for the class.
...
void BSTNode::insert(string d)  // Insert a new node into the tree.
{

	if (treeLevelCounter++ > treeLevelMaximum)							//+1 to lvlCount and if > lvlMax
	{
		treeLevelMaximum++;												//+1 to lvlMax
		/*cout << "insert:   the data to insert is " << d << endl;*/
	}

	/*.compare returns 0 for equal, <0 for shorter or less, >0 for greater or longer strings*/
	if (data.compare(d) == 0)								// This data is already in the tree.
	{
		//cout << "insert:    ---- data " << d << " is already in tree! " << endl;
		return;
	}
	else if (data.compare(d) > 0 && left == NULL)			// This data is a new left child
	{
		left = new BSTNode(d);
		//cout << "insert:  " << d <<  "   was inserted as a left child at level " << treeLevelCounter << endl;
		wordsInserted++;
	}
	else if (data.compare(d) < 0 && right == NULL)			// This data is a new right child
	{
		///////////// DONE COMPLETE ME!
		right = new BSTNode(d);
		//cout << "insert:  " << d <<  "   was inserted as a right child at level " << treeLevelCounter << endl;
		wordsInserted++;
	}
	else if (data.compare(d) > 0)							// This data goes in the left subtree
	{
		///////////// DONE COMPLETE ME!
		//Move from root of subtree to left child and recheck
		(*left).insert(d);
	}
	else if (data.compare(d) < 0)							// This data goes in the right subtree
	{
		///////////// DONE COMPLETE ME!
		//Move from root of subtree to right child and recheck
		(*right).insert(d);
	}
...
void BSTNode::graph(int leftX, int rightX, int y)
{
	...
	if (right != NULL)
	{
		...
		///////////// COMPLETE ME! using a recursive call
		(*right).graph((leftX + rightX) / 2, rightX, y + 40);
	}
} // end graph()
